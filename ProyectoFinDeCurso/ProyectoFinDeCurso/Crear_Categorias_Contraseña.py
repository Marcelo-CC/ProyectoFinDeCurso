...
class CategoriaPasswordsGUI:
    ...
    def create_widgets(self):
        ...
        # Nuevo botón para ver historial de contraseñas
        self.btn_ver_historial = self.create_button(button_frame, "Ver Historial", self.ver_historial_contraseñas)
        self.btn_ver_historial.grid(row=2, column=0, padx=5)
    ...
    def agregar_contraseña(self):
        ...
        if seleccion:
            categoria = self.listbox_categorias.get(seleccion[0])
            contrasena = simpledialog.askstring("Nueva Contraseña",
                                                f"Ingresa la contraseña para la categoría '{categoria}':")

            if contrasena:
                # Llamar la función correctamente usando self.id_usuario
                try:
                    DB.insertar_contrasena(self.id_usuario, categoria, contrasena)
    ...
    def ver_contraseñas(self):
        ...
        if seleccion:
            ...
            if contraseñas:
                # Convertir cada tupla a una cadena de texto
                contraseñas_texto = "\n".join(
                    [f"Contraseña: {contrasena[0]}\nFecha: {contrasena[1]}" for contrasena in contraseñas])
    ...
    def ver_historial_contraseñas(self):
        seleccion = self.listbox_categorias.curselection()
        if seleccion:
            categoria = self.listbox_categorias.get(seleccion[0])
            historial = DB.obtener_historial_contraseñas(self.id_usuario, categoria)

            if historial:
                historial_texto = ""
                for h in historial:
                    # Asegúrate de que h[0] sea la contraseña y h[1] la fecha, si tu estructura de historial es diferente ajusta esto
                    historial_texto += f"Contraseña: {h[0]}\nFecha: {h[1]}\n\n"
                messagebox.showinfo(f"Historial de Contraseñas de '{categoria}'", historial_texto)
            else:
                messagebox.showinfo("Sin historial",
                                    f"No hay historial de contraseñas para la categoría '{categoria}'.")
        else:
            messagebox.showwarning("Selección inválida", "Debes seleccionar una categoría para ver el historial.")
